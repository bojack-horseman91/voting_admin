...
const UpazillaAdmin: React.FC<Props> = ({ upazillaId }) => {
  ...
  const handleDeleteUnion = async (id: string, name: string) => {
      if(window.confirm(`Are you sure you want to delete "${name}"? This action cannot be undone.`)) {
          try {
              await DB.deleteUnion(id, upazillaId);
              // If the deleted union was selected, reset selection
  ...
  const handleDeleteCenter = async (id: string, name: string) => {
      if(window.confirm(`Delete voting center "${name}"?`)) {
          try {
              await DB.deleteCenter(id, upazillaId);
              setSuccessMsg(`"${name}" deleted successfully.`);
              // If we were editing this center, cancel edit
