...
import { Plus, MapPin, Users, Upload, ExternalLink, ShieldCheck, FileText, BrainCircuit, Edit2, CheckCircle2, X } from 'lucide-react';
...
const UpazillaAdmin: React.FC<Props> = ({ upazillaId }) => {
  ...
  const handleEditCenter = (center: VotingCenter) => {
      setEditingCenterId(center.id);
      setCenterForm({ ...center });
      // If editing, the preview is the existing URL
      setImagePreviewUrl(center.imageUrl || '');
      setSelectedImageFile(null); // Reset new file selection
  ...
  const generateSecurityBrief = async (center: VotingCenter) => {
      const plan = await AI.generateSecurityPlan(center);
                ...
                {unions.length === 0 ? (
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                            {centers.map(center => (
                                <div key={center.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 flex gap-4 transition-all hover:shadow-md">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start">
                                            <h4 className="text-lg font-bold text-gray-900 truncate">{center.name}</h4>
                                            {center.googleMapLink && (
                                                <a href={center.googleMapLink} target="_blank" rel="noreferrer" className="text-blue-500 hover:text-blue-700" title="Open Map">
                                                    <ExternalLink className="w-5 h-5" />
                                                </a>
                                            )}
                                        </div>
                                        <p className="text-sm text-gray-500 mb-3">{center.location}</p>
                                        
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-xs">
                                            <div className="bg-blue-50 p-2 rounded text-blue-800">
                                                <strong>{center.presidingOfficer.position}:</strong> {center.presidingOfficer.name} <br/>
                                                <span className="opacity-75">{center.presidingOfficer.phone}</span>
                                            </div>
                                            <div className="bg-blue-50 p-2 rounded text-blue-800">
                                                <strong>{center.assistantPresidingOfficer.position}:</strong> {center.assistantPresidingOfficer.name} <br/>
                                                <span className="opacity-75">{center.assistantPresidingOfficer.phone}</span>
                                            </div>
                                            <div className="bg-red-50 p-2 rounded text-red-800 col-span-1 sm:col-span-2">
                                                <strong>{center.policeOfficer.position}:</strong> {center.policeOfficer.name} <br/>
                                                <span className="opacity-75">{center.policeOfficer.phone}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 flex justify-between items-center border-t border-gray-100 pt-3">
                                            ...
                                            >
                                                <BrainCircuit className="w-3 h-3" /> AI Plan
                                            </button>
                                            
                                            <button
                                                onClick={() => handleEditCenter(center)}
                                                className="text-xs flex items-center gap-1 text-amber-600 hover:text-amber-800 font-medium px-2 py-1 rounded hover:bg-amber-50 transition-colors"
                                            >
